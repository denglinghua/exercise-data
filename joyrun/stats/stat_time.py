...
def every_week(df:pd.DataFrame):
    data = df[['id', 'week_no', 'distance']]
    data = data.groupby("id").agg({'week_no':'nunique','distance':'sum'})
...
def days(df:pd.DataFrame):
    ...
    data = data.groupby("id")['end_date'].nunique()
...
def time(df:pd.DataFrame):
    data = df[['id', 'time']]
    #data = data.groupby("id").sum('time')
    # timedelta can't be summed in this way 'sum()'
    data = data.groupby('id').agg({'time': 'sum'})
    data = data.reset_index()
...
def morning_run(df:pd.DataFrame):
    ...
    data = data.groupby('id').agg({'end_time':'count','start_time':'mean'})
    data = data.reset_index()
