...
from core.group_by import RangeGroupBy, ValueGroupBy
from analysis.common import is_running, is_swimming, is_cycling
...
def _run_pace_group_set():
    ...
    return _range_group_set(title, col, group_by, is_running).set_xtitle(lang.run_pace_unit)
...
def _run_cadence_group_set():
    ...
    return _range_group_set(title, col, group_by, is_running).set_xtitle(lang.steps_per_min)
...
def _run_stride_group_set():
    ...
    return _range_group_set(title, col, group_by, is_running).set_xtitle(lang.m)
...
def _cycling_distance_group_set():
    ...
    return _range_group_set(title, col, RangeGroupBy(20, 100, 20), is_cycling).set_xtitle(lang.km)
